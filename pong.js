...
let p1_key, p2_key
var audio = new Audio('trapkick.wav');
var audio2 = new Audio('p1.mp3');
var audio3 = new Audio('p2.mp3');
...
function setup() {
    ...
    // inicializa as posi��es y do p1 e do p2 para metade da tela
    p1_y = p2_y = (h / 2) - (p_h / 2)
...
function loop() {
    ...
    //Verifica se a bola est� colidindo com o barra do player 1
    if (ball_x >= p1_x && ball_x <= p1_x + 10 && ball_y >= p1_y && ball_y <= p1_y + p_h) {
        ball_x_orientation = 1
        audio2.play();
    }
    //Verifica se a bola est� colidindo com o barra do player 2
    else if (ball_x >= p2_x && ball_x <= p2_x + 10 && ball_y >= p2_y && ball_y <= p2_y + p_h) {
        ball_x_orientation = -1
        audio3.play();
    }

    // verifica se a bola passou bateu no ch�o ou no teto
...
function drawRect(x, y, w, h, color = "#111") {
    ctx.fillStyle = color
